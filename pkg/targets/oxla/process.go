...
func (p *processor) processCSI(tableName string, rows []*insertData) uint64 {
	...
	MustExec(p._db, genBatchInsertStmt(p.dbName, tableName, cols, dataRows))
...
func genBatchInsertStmt(dbName string, tableName string, cols []string, dataRows [][]interface{}) string {
	insertStmt := bytes.NewBufferString(fmt.Sprintf("INSERT INTO %s.\"%s\"(\"%s\") VALUES ", dbName, tableName, strings.Join(cols, "\",\"")))
...
func (p *processor) Init(_ int, doLoad, hashWorkers bool) {
	...
	p._db = MustConnect(p.driver, p.opts.GetConnectString())
	if hashWorkers {
		p._csi = newSyncCSI()
	} else {
...
func (p *processor) loadExistingTagsInCache(db *sql.DB) {
	p._csi.mutex.Lock()
	res := MustQuery(db, "SELECT * FROM "+p.dbName+".tags")
	tagCols := tableCols[tagsKey]
	ret := p.sqlTagsToCacheLine(res, tagCols)
	for k, v := range ret {
		p._csi.m[k] = v
